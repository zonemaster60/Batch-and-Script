...
rem create config file if not exist
rem *******************************
If Not Exist %config% Call :data_manager 2
...
rem ***********
Call :show_me 0 0
Call :count_num 1 "Analyze: Check the component store for errors."
...
Call :count_num 2 "CheckHealth: Faster but not as thorough."
Call :run_command "dism /online /cleanup-image /checkhealth" 5
) else (
Call :count_num 2 "ScanHealth: Slower but more thorough test."
...
rem ************
Call :show_me 0 0
Call :count_num 3 "Verify: (not repair) the system files."
...
Call :count_num 1 "ResetBase: Will reset the component stores."
Call :run_command "dism /online /cleanup-image /startcomponentcleanup /resetbase" 5
) else (
Call :count_num 1 "Normal: Performs a normal store cleanup."
...
rem **************
Call :show_me 0 0
Call :count_num 2 "RestoreHealth: Restore/Repair the system image health."
Call :run_command "dism /online /cleanup-image /restorehealth" 5
Call :wait_time
rem scan now
rem ********
Call :show_me 0 0
Call :count_num 3 "Scans: And repairs corrupted system files."
...
rem PrintColorAt "3] btnbg=%btnbg% - button background color." 7 10 %btnbg% %txtbg%
rem PrintColorAt "4] btnfg=%btnfg% - button foreground color." 8 10 %btnfg% %txtbg%
rem PrintColorAt "5] schcol1=%schcol1% - scheme color one." 9 10 %schcol1% %txtbg%
rem PrintColorAt "6] schcol2=%schcol2% - scheme color two." 10 10 %schcol2% %txtbg%
rem PrintColorAt "7] schcol3=%schcol3% - scheme color three." 11 10 %schcol3% %txtbg%
rem PrintColorAt "8] schcol4=%schcol4% - scheme color four." 12 10 %schcol4% %txtbg%
rem PrintColorAt "9] txtbg=%txtbg% - text background color." 13 10 %txtbg% 7
rem PrintColorAt "10] txtfg=%txtfg% - text foreground color." 14 9 %txtfg% %txtbg%
rem PrintCenter "(Only values that can be edited are shown here.)" 16 14 %txtbg%
Call :click_next
...
rem button matrix
rem *************
rem MouseCmd 5,6,14,6 5,7,14,7 6,16,%len1%,16 6,18,%len2%,18 6,20,%len3%,20 6,22,%len4%,22
...
If %2 EQU 1 ( 
rem PrintColorAt "David Scouten [c2023] zonemaster@yahoo.com" 25 20 %schcol2% %txtbg%
)
rem CursorHide
GOTO:EOF
...
Cmd /c %1
rem PrintReturn
rem PrintColorAt "Finish: %TIME%" 23 2 14 13
If %ErrorLevel% LSS 1 (
rem PrintReturn
...
rem MouseCmd 36,25,44,25
If %result% EQU 1 Call :make_button "[ NEXT>> ]" 25 36 1 10 %schcol2% %btntime% %txtbg%
GOTO:EOF
...
If %logson% EQU True Echo [%TIME%: Loaded Configuration: '%config%']>>%logfile%
GOTO:EOF
...
If %logson% EQU True Echo [%TIME%: Saved Configuration: '%config%']>>%logfile%
GOTO:EOF
...
Set analyze=False
Set repair=False
If %logson% EQU True Echo [%TIME%: Reset Configuration: '%config%']>>%logfile% 
Call :data_manager 2
GOTO:EOF
