...
if not exist "%backuppath%\regbackup0\nul" mkdir "%backuppath%\regbackup0"
for %%k in (lm cu cr u cc) do call :ExpReg %%k
Exit /B 0

:ExpReg
reg.exe export hk%1 "%backuppath%\regbackup0\hk%1.reg" /y
if "%errorlevel%"=="1" (
...
:ImpReg
reg.exe import "%backuppath%\regbackup0\hk%1.reg"
if "%errorlevel%"=="1" (
  echo ^>^> Export --hk%1-- Failed.
) else (
  echo ^>^> Export --hk%1-- Succeeded.
